...
describe('TodoListApplication', () => {
    ...
    it('gets a count of todos', async function () {
        await helpers_1.givenTodoInstance(todoRepo, {
            title: 'say hello',
    ...
    context('when dealing with a single persisted todo', () => {
        ...
        it('returns 404 when updating a todo that does not exist', () => {
            return client.patch('/todos/99999').send(helpers_1.givenTodo()).expect(404);
        });
    ...
    it('includes TodoList in query result', async () => {
        ...
        testlab_1.expect(response.body[0]).to.deepEqual({
            ...testlab_1.toJSON(todo),
            todoList: testlab_1.toJSON(list),
        });
    });
});
